...
%% center lime
%clear all
sol=coco_read_solution('Branch 1_30',1);
DOF1=sol{3,2}.Segments{1,1}.DOF;
DOF2=sol{3,2}.Segments{1,2}.DOF;
for i=1:2
...
%clear all
global Segments
% sol=coco_read_solution('Test6',37);

for i=1:2
    for j = 1:size(Segments{i}.Nodes,1)-1
        for k=-4:1:4
            Rx(j,k+5)=Segments{i}.Nodes(j,1) +k.*1/250.*Segments{i}.m1(j,1);
            Ry(j,k+5)=Segments{i}.Nodes(j,2) +k.*1/250.*Segments{i}.m1(j,2);
            Rz(j,k+5)=Segments{i}.Nodes(j,3) +k.*1/250.*Segments{i}.m1(j,3);
        end
    end
    surf(Rx,Ry,Rz,'FaceColor','b','EdgeColor','none');hold on
end
